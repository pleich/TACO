...
--------------------------------------------------------------------------------
TypeOk == nsntYesCF \in Nat 
    /\ nsntNoCF \in Nat
    /\ ProcessesLocations \in [Processes -> {"locNO", "locYES", "locSE", "locSEFD", "locCMT","locABR", "locCR"}] 


Init == ProcessesLocations \in [Processes -> {"locNO", "locYES"}] 
    /\ nsntNoCF = 0
    /\ nsntYesCF = 0
...
Rule1(p) == ProcessesLocations[p] = "locYES"
    ...
    /\ UNCHANGED << nsntNoCF>>
--------------------------------------------------------------------------------
Rule2(p) == ProcessesLocations[p] = "locSE"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locSEFD"]
...
Rule3(p) == ProcessesLocations[p] = "locSE"
    ...
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>               
--------------------------------------------------------------------------------
Rule4(p) == ProcessesLocations[p] = "locSEFD"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locABR"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>
--------------------------------------------------------------------------------
...
Rule6(p) == ProcessesLocations[p] = "locNO"
    ...
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>          
--------------------------------------------------------------------------------
Rule7(p) == ProcessesLocations[p] = "locYES"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locCR"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>
--------------------------------------------------------------------------------
Rule8(p) == ProcessesLocations[p] = "locSE"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locCR"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>  
--------------------------------------------------------------------------------
Rule9(p) == ProcessesLocations[p] = "locSEFD"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locCR"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>
--------------------------------------------------------------------------------
Rule10(p) == ProcessesLocations[p] = "locCMT"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locCR"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>
--------------------------------------------------------------------------------
Rule11(p) == ProcessesLocations[p] = "locABR"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locCR"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>
--------------------------------------------------------------------------------
Rule12(p) == ProcessesLocations[p] = "locSE"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locSE"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>
--------------------------------------------------------------------------------
Rule13(p) == ProcessesLocations[p] = "locSEFD"
    /\ ProcessesLocations' = [ProcessesLocations EXCEPT ![p] = "locSEFD"]
    /\ UNCHANGED << nsntNoCF, nsntYesCF>>
--------------------------------------------------------------------------------
...
Next == \E p \in Processes: Rule0(p) \/ Rule1(p) \/ Rule2(p) \/ Rule3(p)
    ...
    \/ Rule10(p) \/ Rule11(p) \/ Rule12(p) \/ Rule13(p) \/ Rule14(p) \/ Rule15(p)
...
NumInlocSEFD == Cardinality({p \in Processes : ProcessesLocations[p] = "locSEFD"})
...
validity ==
  (NumInlocNO = 0) \/ [] (NumInlocCMT = 0)

nontriv ==
  ((<>[] ((NumInlocNO = 0) /\ (NumInlocYES = 0) /\ (NumInlocSEFD = 0)
         /\ (((nsntNoCF < 1) \/ (NumInlocSE = 0))
             /\ (((nsntYesCF < N) \/ (nsntNoCF >= 1)) \/ (NumInlocSE = 0))))
    /\ [] ((NumInlocSEFD = 0) /\ (NumInlocCR = 0))
    /\ (NumInlocNO = 0))
  =>
  <> ((NumInlocNO = 0) /\ (NumInlocYES = 0) /\ (NumInlocSE = 0)
       /\ (NumInlocSEFD = 0) /\ (NumInlocABR = 0) /\ (NumInlocCR = 0)))
...
termination1 ==
  ((<>[] ((NumInlocNO = 0) /\ (NumInlocYES = 0) /\ (NumInlocSEFD = 0)
    ...
    /\ [] (NumInlocCR = 0))
  =>
  <> ((NumInlocNO = 0) /\ (NumInlocYES = 0) /\ (NumInlocSE = 0) /\ (NumInlocSEFD = 0)))
